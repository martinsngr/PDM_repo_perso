...
# -----------------------------
# The yarn is crimped, so the bounding-box centre can lie outside it:
# take the centre of the cross-section in a thin slice at mid-length
xcat = 0.5 * (min_coords[0] + max_coords[0])
slice_pts = points[np.abs(points[:, 0] - xcat) < 0.02 * delta[0]]
ycat = 0.5 * (slice_pts[:, 1].min() + slice_pts[:, 1].max())
zcat = 0.5 * (slice_pts[:, 2].min() + slice_pts[:, 2].max())
